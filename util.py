...
def recognize_DGL_algdep(J, degree, tolerance=0.9, prime_bound=None, roots_of_unity=None, outfile=None):
    K = J.parent()
    p = K.prime()
    ...
    for i, zz in enumerate(roots_of_unity):
        ...
        if height_poly < height_threshold:
            ...
            fwrite("# %s (Norm = %s)\t (J * exp(2 pi i %s / %s\t (height=%s)"%(ff.factor(), nrm.factor(), i, lcm(12,(p**2-1)), height_poly), outfile)
