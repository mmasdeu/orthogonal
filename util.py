...
def recognize_DGL_algdep(J, degree, tolerance=0.9, roots_of_unity=None, outfile=None):
    ...
    for Jz, i in Jzlist0:
        ...
        try:
            Jzlist.append((our_sqrt(Jz),i, 2))
            Jzlist.append((our_nroot(Jz, 4),i,4))
        except ValueError:
            continue
    for Jz, i, pw in Jzlist:
        ...
        if height_poly < height_threshold:
            ...
            fwrite("# %s (Norm = %s)\t ((J * exp(2 pi i %s / %s))^(1/%s)\t (height=%s)"%(ff.factor(), nrm.factor(), i, lcm(12,(p**2-1)), clist_ans, height_poly), outfile)
            ffsmall = sage_eval(str(gp.polredabs(ff)),locals={'x' : x})
            fwrite("Number field generated by J has primitive polynomial = %s"%ffsmall, outfile)
# return J_alg.minpoly(), remainder, fact, clist_ans, gp.polredabs(J_alg.parent().polynomial()), phi, K_to_Cp  # DEBUG - didn't check that they match...
            return Jz, ff, ffsmall, nrm, i, lcm(12,(p**2-1)), clist_ans
...
def recognize_DGL_lindep(J, L, prime_list, Cp = None, units=None, outfile=None, **kwargs):
    ...
    for phi in phi_list:
        ...
        if class_number is not None:
            found = False
            for pp, _ in L.ideal(p).factor():
                unif = phi(pp.gens_reduced(proof=False)[-1])
                if unif.valuation() != 0:
                    found = True
                    break
            assert found
            lp = -Cp(p/unif).log()
            Vlogs = [K_to_Cp(J.log(lp))]
            ...
            for ell in prime_list:
                ...
                Vlogs += [phi(g0).log(lp) for g0 in ell_set]
        ...
        if not algebraic:
            ...
            return J_alg.minpoly(), remainder, clist_ans, gp.polredabs(J_alg.parent().polynomial())  # DEBUG - didn't check that they match...
