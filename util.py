...
def our_lindep(V,prec = None, base=None, **kwargs):
    ...
    if kwargs.get('debug_matrix', None) is not None:
        return M
    if algorithm == 'magma':
...
def recognize_DGL_algdep(J, degree, tolerance=0.9, roots_of_unity=None, outfile=None):
    ...
    for Jz, i, pw in Jzlist:
        ...
        if height_poly < height_threshold:
            ...
            fwrite(f'# {ff.factor()} (Norm = {nrm.factor()})\t (J * exp(2 pi i {i} / {lcm(12,(p**2-1))}))^(1/{pw})\t{str(clist_ans)} (height={height_poly})', outfile)
            try:
    ...
    for phi in phi_list:
        ...
        if not algebraic:
            ...
            try:
                ...
                pass
            return J_alg.minpoly(), remainder, clist_ans, ffsmall  # DEBUG - didn't check that they match...
