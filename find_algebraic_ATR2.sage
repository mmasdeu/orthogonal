...
for D, n in [(D, n) for D in Dvalues for n in nvalues]:
    ...
    ans2 = recognize_DGL_algdep(Jtau,2*hE, outfile=outfile)
    fwrite(str(ans2),outfile)
    ans4 = recognize_DGL_algdep(Jtau,4*hE, outfile=outfile)
    fwrite(str(ans4),outfile)
    ans8 = recognize_DGL_algdep(Jtau,8*hE, outfile=outfile)
    fwrite(str(ans8),outfile)
    fwrite('...', outfile)
